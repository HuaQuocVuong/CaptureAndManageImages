...
class _CameraScreenState extends State<CameraScreen>
  ...
  late AudioPlayer _audioPlayer; // Trình phát âm thanh để phát tiếng chụp ảnh
  ...
  // THÊM PHƯƠNG THỨC NÀY VÀO ĐÂY - sau các biến và trước các phương thức khác
  Future<void> _checkPhotoStatus(int photoId) async {
    try {
      final photo = await _photoDao.getPhotoById(photoId.toString());
      if (photo != null) {
        debugPrint('=== KIỂM TRA ẢNH ID $photoId ===');
        debugPrint('  Status: ${photo.status}');
        debugPrint('  Product ID: ${photo.productId}');
        debugPrint('  Title: ${photo.title}');
        debugPrint('  Description: ${photo.description}');
        debugPrint('  Price: ${photo.price}');
        debugPrint('  Category: ${photo.category}');
        debugPrint('  Note: ${photo.note}');
      } else {
  ...
  Future<void> _playShutterSound() async {
    ...
    try {
      // Cách 1: Dùng asset (khuyên dùng)
      await _audioPlayer.play(AssetSource('images/camera-shutter-02.mp3'));
    } catch (e) {
      // Không hiển thị lỗi cho người dùng vì âm thanh không quan trọng
  ...
  */
  ...
  Future<void> _handleCapture() async {
    ...
    try {
      await _playShutterSound(); // Phát âm thanh chụp ảnh
  ...
  Future<void> _processImageInBackground(PhotoTask task) async {
    ...
    try {
      ...
      // Load lại danh sách ảnh từ database để đồng bộ
      if (mounted && !_disposed) {
